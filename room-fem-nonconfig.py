...
# ==== PARAMETERS ============================================================
LX, LY, LZ = 5.0, 4.0, 2.7          # room dimensions (m)
C0 = 343.0                          # speed of sound (m/s)
RHO_AIR = 1.21                      # air density (kg/m^3)

SRC_Y, SRC_Z = LY / 2, 1.2          # source position on the x=0 wall
SRC_R = 0.09                        # source patch radius (m)
SRC_VELOCITY = 0.29                 # source normal surface velocity (m/s)

FREQ_MIN, FREQ_MAX = 20, 150        # sweep range (Hz)
N_SUBSTEPS = 130                    # one substep per solved frequency

ELEMS_PER_WAVELENGTH = 10           # mesh density target at FREQ_MAX
ESIZE = C0 / (FREQ_MAX * ELEMS_PER_WAVELENGTH)

ALPHA_WALL = 0.10                   # normal-incidence wall absorption
_r = np.sqrt(1.0 - ALPHA_WALL)
Z_WALL = RHO_AIR * C0 * (1.0 + _r) / (1.0 - _r)   # wall impedance (Pa*s/m)

LISTENER_XYZ = (LX / 2, LY / 2, 1.2)
PLANE_HEIGHTS_Z = [0.3, 1.2, 2.4]    # floor / listener height / near-ceiling
P_REF = 20e-6                       # 0 dB SPL reference pressure (Pa)

NUM_PEAKS = 4                       # strongest resonances to plot
NUM_DIPS = 2                        # deepest nulls to mark
MODE_MATCH_TOL = 3.0                # Hz, peak-to-analytical-mode window
