...
---
## Η πηγή monopole και η έξοδός της
...
καμία δύναμη, μάζα ή δυσκαμψία πίσω από αυτή την κίνηση· η ταχύτητα
απλώς ορίζεται ως δεδομένο.

### Γιατί ονομάζεται "monopole"

Επειδή η επιφάνεια της πηγής είναι μικρή σε σχέση με τα μήκη κύματος που
μελετώνται, ακτινοβολεί σχεδόν ομοιόμορφα προς όλες τις κατευθύνσεις,
όπως μια εξιδανικευμένη σημειακή πηγή (monopole). Η συνθήκη εγκυρότητας
αυτής της προσέγγισης είναι `ka ≪ 1`, όπου `k = 2πf/c` ο κυματάριθμος και
`a = SRC_R` η ακτίνα της πηγής. Στη μέγιστη συχνότητα της σάρωσης
(`FREQ_MAX = 150` Hz), `k ≈ 2.75` rad/m, άρα `ka ≈ 2.75 × 0.09 ≈ 0.25` —
αρκετά μικρό ώστε η προσέγγιση monopole να ισχύει σε όλο το εύρος της
σάρωσης.

### Τι αντιπροσωπεύει η "έξοδος" (`SRC_VELOCITY`)

Το `SRC_VELOCITY` είναι το πλάτος της κάθετης ταχύτητας της επιφάνειας
(σε m/s) — αντιστοιχεί χονδρικά στην κίνηση ενός κώνου ηχείου, αλλά
**χωρίς καμία εξάρτηση από τη συχνότητα** και χωρίς πραγματική δύναμη
οδήγησης πίσω της. Σε ένα πραγματικό ηχείο (ή στο μοντέλο FSI,
`room-fem-fsi.py`), η ταχύτητα θα προέκυπτε από την επίλυση δύναμη =
μάζα × επιτάχυνση + δυσκαμψία × μετατόπιση + απόσβεση, και θα ποίκιλλε
ανάλογα με τη συχνότητα, με δικό της συντονισμό. Εδώ, αντίθετα, η
ταχύτητα επιβάλλεται απευθείας — μια σκόπιμη απλοποίηση που απομονώνει
την ακουστική συμπεριφορά του δωματίου από την αβεβαιότητα των
μηχανικών ιδιοτήτων ενός πραγματικού ηχείου.

### Ενδεικτικές τιμές

Η προεπιλεγμένη τιμή `SRC_VELOCITY = 0.29` m/s αντιστοιχεί σε μέτρια
στάθμη αναπαραγωγής. Ένας γρήγορος έλεγχος λογικής: η μέγιστη ταχύτητα
ενός κώνου είναι περίπου `v = ω · Xmax`, όπου `Xmax` το όριο εκτροπής
του ηχείου. Στα 50 Hz, με τυπικό `Xmax ≈ 5` mm, `v_max ≈ 2π · 50 · 0.005
≈ 1.6` m/s — άρα η τιμή `0.29` m/s βρίσκεται άνετα κάτω από αυτό το
όριο, αντιπροσωπεύοντας μια ρεαλιστική, μη ακραία στάθμη ήχου.
